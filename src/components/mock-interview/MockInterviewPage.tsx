...
import type { MockInterview, InterviewQuestion, QuestionEvaluation, InterviewReport } from '../../types';
import {
  Play,
  ChevronRight,
  Calendar,
...
async function generateReport(
  ...
  scores: { overall: number; technical: number; communication: number; confidence: number }
...
export default function MockInterviewPage() {
  ...
  const completeInterview = async (evaluations: (QuestionEvaluation | null)[]) => {
    ...
    try {
      ...
      report = {
        summary: 'Interview completed successfully. Review your individual question feedback for improvement areas.',
        strengths: ['Completed all questions'],
        areasForImprovement: ['Review evaluation feedback'],
    ...
    const categoryQuestions = getQuestionsForCategory(session.category);
    const questionsData = categoryQuestions.map((q, idx) => ({
      id: q.id,
      question: q.question,
      category: q.category,
  ...
  if (session && session.started && !session.completed) {
    ...
    // If we have an evaluation to show
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className={`stat-card bg-gradient-to-br ${getScoreBg(currentEvaluation.score * 10)} space-y-4`}>
              <div className="flex items-center justify-between">
                ...
                <div className={`text-5xl font-bold ${getScoreColor(currentEvaluation.score * 10)}`}>
            ...
            {currentEvaluation.correctPoints.length > 0 && (
              <div className="stat-card space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-xl font-semibold text-white">What You Covered Well</h3>
                </div>
            ...
            {currentEvaluation.missingPoints.length > 0 && (
              <div className="stat-card space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-xl font-semibold text-white">Points You Missed</h3>
                </div>
                <ul className="space-y-2">
            ...
            <div className="stat-card space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xl font-semibold text-white">Ideal Answer</h3>
              </div>
              <div className="p-4 bg-white/5 rounded-lg border border-white/10">
                <p className="text-gray-300 whitespace-pre-wrap">{currentEvaluation.idealAnswer}</p>
              </div>
            </div>
            ...
            <div className="flex gap-4">
              ...
              <button
                onClick={() => {
                  // Clear the evaluation view and move to next question
                  const newEvaluations = [...session.evaluations];
                  if (session.currentQuestionIndex < categoryQuestions.length - 1) {
                    setSession({
                      ...session,
              ...
              >
                {session.currentQuestionIndex === categoryQuestions.length - 1
                  ? 'Finish Interview'
                  : 'Next Question'}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-gray-400">
                Question {session.currentQuestionIndex + 1} of {categoryQuestions.length}
              </span>
            </div>
          ...
          <div className="stat-card space-y-6">
            <div className="space-y-4">
              ...
              <p className="text-gray-400 text-sm">
                Take your time to provide a comprehensive answer. AI will evaluate your response.
              </p>
            </div>
            ...
            <div className="flex gap-4 justify-between">
              <button onClick={resetSession} className="btn-secondary" disabled={evaluating}>
                Exit Interview
              </button>
              <button
        ...
        <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="flex items-center justify-between">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-400 mt-1">
                    <span className="flex items-center gap-1">
                      <Clock size={14} />
                      {Math.floor(session.startTime.getTime() / 1000 % 60)} min
                    </span>
              ...
              <div className="text-right">
                ...
                <p className="text-gray-400 text-sm mt-1">
                  {scores.overall >= 80 ? 'Excellent!' : scores.overall >= 60 ? 'Good effort!' : 'Keep practicing!'}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="stat-card space-y-3">
              ...
              <p className={`text-3xl font-bold ${getScoreColor(scores.technical)}`}>
                {scores.technical}%
              </p>
              <div className="w-full bg-white/10 rounded-full h-2">
                <div
            ...
            <div className="stat-card space-y-3">
              ...
              <p className={`text-3xl font-bold ${getScoreColor(scores.communication)}`}>
            ...
            <div className="stat-card space-y-3">
              ...
              <p className={`text-3xl font-bold ${getScoreColor(scores.confidence)}`}>
                {scores.confidence}%
              </p>
              <div className="w-full bg-white/10 rounded-full h-2">
                <div
            ...
            <div className="stat-card space-y-6">
              ...
              {showDetailedReport && (
                <div className="space-y-6">
                  {/* Summary */}
                  <div className="p-4 bg-white/5 rounded-lg border border-white/10">
                    <p className="text-gray-300 leading-relaxed">{session.report.summary}</p>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <ul className="space-y-2">
                      {session.report.recommendations.map((rec, idx) => (
                        <li key={idx} className="flex gap-2 text-gray-300">
                          <ChevronRight className="text-primary-400 shrink-0 mt-0.5" size={16} />
                          <span>{rec}</span>
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <p className="text-white font-medium line-clamp-2">
                        {categoryQuestions[idx]?.question || 'Question'}
                      </p>
                    </div>
                  </div>
          ...
          <div className="flex gap-4">
            ...
            <button
              onClick={() => startInterview(session.category)}
              className="btn-primary flex-1 flex items-center justify-center gap-2"
            >
              <Play size={18} />
              Retake Interview
          ...
          {categories.map((category) => {
            ...
            return (
              <button
                ...
                className="stat-card group hover:border-primary-500/50 space-y-4 text-left"
              >
                <div className="flex items-center justify-between">
                ...
                <p className="text-gray-400">
                  {category === 'Technical'
                    ? 'Test your technical knowledge and coding concepts'
                    : category === 'Behavioral'
                      ? 'Practice answering HR and behavioral questions'
                      : 'Design large-scale systems and architecture'}
                </p>
                <div className="flex items-center gap-2 text-primary-400 text-sm font-medium">
                  <span>{categoryQuestions.length} Questions</span>
                  <span className="text-gray-500">with AI evaluation</span>
                </div>
              </button>
            );
          })}
              ...
              {interviews.map((interview) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4 flex-1">
                      ...
                      <div className="flex-1">
                        <p className="text-white font-semibold">
                          {interview.questions[0]?.category || 'Interview'} Interview
                        </p>
                        <div className="flex items-center gap-4 text-sm text-gray-400 mt-1">
                          <span className="flex items-center gap-1">
                            <Calendar size={14} />
                    ...
                    <div className="text-right">
                      <p className={`text-2xl font-bold ${getScoreColor(interview.overall_score)}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="stat-card space-y-3">
            ...
            <ul className="text-sm text-gray-400 space-y-2">
              <li className="flex gap-2">
                <span className="text-primary-400 font-bold">*</span>
                <span>Review your evaluations</span>
              </li>
              <li className="flex gap-2">
                <span className="text-primary-400 font-bold">*</span>
                <span>Study ideal answers</span>
              </li>
              <li className="flex gap-2">
                <span className="text-primary-400 font-bold">*</span>
